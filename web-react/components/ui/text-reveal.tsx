...
export interface MaskedTextRevealProps {
  ...
  duration?: number;
  /**
   * Play as soon as the text has been measured, without waiting for the element
   * to enter the viewport. Use for above-the-fold copy — a background tab never
   * fires the first IntersectionObserver callback, which would otherwise leave
   * the text hidden until the first scroll.
   * @default false
   */
  immediate?: boolean;
  /**
...
export function MaskedTextReveal({
  ...
  const shouldAnimate = (immediate || isInView) && isReady && !prefersReducedMotion;
